...
def generator(gdata, img_size, batch_size, c_dim, num_filter):
    ...
    stddev = 0.001
    with tf.variable_scope('g_conv1',reuse=None) as scope:
    ...
    with tf.variable_scope('g_conv2',reuse=None) as scope:
...
def build_model(img_size, batch_size=100, num_filter=16, c_dim=1, leak=0.1):
    ...
    noise_images = tf.placeholder(tf.float32, [batch_size] 
            + [img_size, img_size, c_dim], name='n_images')
...
flags = tf.app.flags
flags.DEFINE_float("learning_rate", 0.0001, "Learning rate for adam [0.0002]")
flags.DEFINE_float("beta1", 0.5, "Learning rate for adam [0.0002]")
flags.DEFINE_integer("epoch", 1000, "Epoch to train [10]")
flags.DEFINE_string("dataset", "stl10_binary", "The name of dataset []")
flags.DEFINE_integer("batch_size", 100, "The size of batch images [64]")
flags.DEFINE_integer("image_size", 96, "The size of image to use (will be center cropped) [10.]")
...
def read_images(c_dim):
    ...
    noise_data = glob(os.path.join("./data", FLAGS.dataset, "n_images", "*.png"))
    ...
    for epoch in range(FLAGS.epoch):
        ...
        for idx in range(0, num_batch):
            ...
            if np.mod(counter, 100) == 1:
                ...
                #save_images(sample_z, [10, 10], '{}/noise_{:02d}_{:04d}.png'.format('./sample3', epoch, idx))
                save_images(samples, [10, 10], '{}/denoise_{:02d}_{:04d}.png'.format('./sample3', epoch, idx))
                #save_images(sample_images, [10, 10], './{}/train_{:02d}_{:04d}.png'.format('./sample3', epoch, idx))
